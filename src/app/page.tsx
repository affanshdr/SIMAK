...
const documentDescriptions: Record<DocType, string> = {
  ...
  "Surat Pengantar RT/RW":
    "Surat pengantar dari RT/RW setempat (format PDF/DOC, max 2MB)",
  "Surat Permohonan Bermaterai":
    "Surat permohonan bermaterai 6000 (format PDF/DOC, max 2MB)",
  "Izin Usaha": "Dokumen izin usaha jika ada (format PDF/JPG, max 2MB)",
  "Pas Foto 3X4 (latar Merah)":
    "Pas foto terbaru ukuran 3x4 dengan latar belakang merah (format JPG/PNG, max 2MB)",
  "Surat Pernyataan tidak mampu dari RT/RW":
    "Surat pernyataan tidak mampu dari RT/RW (format PDF/DOC, max 2MB)",
  "Rekening listrik/air 3 bulan terakhir":
    "Rekening listrik/air 3 bulan terakhir (format PDF/DOC, max 2MB)",
};

// Inisialisasi state untuk file
const initialFileState = Object.keys(documentDescriptions).reduce(
  (acc, doc) => {
    return { ...acc, [doc]: null };
  },
...
const initialPreviewState = Object.keys(documentDescriptions).reduce(
  (acc, doc) => {
    return { ...acc, [doc]: null };
  },
...
export default function FormPengajuanSurat() {
  ...
  const [selectedFiles, setSelectedFiles] =
    useState<Record<DocType, File | null>>(initialFileState);
  const [previews, setPreviews] =
    useState<Record<DocType, string | null>>(initialPreviewState);
  ...
  useEffect(() => {
    if (formData.noNIK.length === 16) {
      const checkNik = async () => {
        try {
          const response = await fetch(`/api/check?noNIK=${formData.noNIK}`);
          if (!response.ok) throw new Error("Failed to check NIK");
          ...
          if (data.success) {
            ...
            if (data.exists && data.data) {
              ...
              setFormData((prev) => ({
                ...prev,
                namaLengkap: data.data.nama || prev.namaLengkap,
                noKK: data.data.no_kk || prev.noKK,
              }));
            } else {
              setWargaData(null);
            }
          } else {
            throw new Error(data.message || "Failed to check NIK");
          }
        } catch (error) {
          ...
          setWargaData(null);
          toast.error(
            error instanceof Error ? error.message : "Error checking NIK"
          );
        }
      };
  ...
  useEffect(() => {
    if (formData.noKK.length === 16) {
      const checkKk = async () => {
        try {
          const response = await fetch(`/api/check?noKK=${formData.noKK}`);
          if (!response.ok) throw new Error("Failed to check KK");
          const data = await response.json();

          if (data.success) {
            setKkValid(data.exists);
            if (data.exists && data.data && !wargaData) {
              setWargaData(data.data);
            }
          } else {
            throw new Error(data.message || "Failed to check KK");
          }
        } catch (error) {
          console.error("Error checking KK:", error);
          setKkValid(false);
          toast.error(
            error instanceof Error ? error.message : "Error checking KK"
          );
        }
      };

      const debounceTimer = setTimeout(checkKk, 500);
      return () => clearTimeout(debounceTimer);
    } else {
      setKkValid(null);
    }
  }, [formData.noKK, wargaData]);
  ...
  const checkWargaExists = async (
    noKK: string,
    noNIK: string
  ): Promise<boolean> => {
    try {
      const response = await fetch(`/api/check?noKK=${noKK}&noNIK=${noNIK}`);
      if (!response.ok) {
        throw new Error("Gagal memeriksa data warga");
      }
      const data = await response.json();
      return data.exists && data.is_match;
    } catch (error) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (formData.noNIK.length !== 16 || formData.noKK.length !== 16) {
        throw new Error("NIK dan Nomor KK harus 16 digit");
      }

      // Check if all required documents are uploaded
      const missingDocs = requiredDocuments.filter(
        (doc) => !selectedFiles[doc]
      );
      if (missingDocs.length > 0) {
        throw new Error(
          `Dokumen berikut masih diperlukan: ${missingDocs.join(", ")}`
        );
      }

      // Check if warga exists and NIK/KK match
      const wargaExists = await checkWargaExists(formData.noKK, formData.noNIK);
      if (!wargaExists) {
  ...
  const validateFile = (file: File, docType: DocType): boolean => {
    ...
    if (
      (docType === "KTP" ||
        docType === "KK" ||
        docType === "Pas Foto 3X4 (latar Merah)") &&
      !isImage
    ) {
    ...
    if (
      (docType.includes("Surat") || docType === "Izin Usaha") &&
      !(isPDF || isDoc || isImage)
    ) {
  ...
  const requiredDocuments = selectedLetter
    ? documentRequirements[selectedLetter].docs
    : [];
      ...
      <div className="container mx-auto px-4 py-8 mt-8">
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div className="bg-orange-500 p-6 text-white">
            ...
            <p className="mt-2">
              Silakan lengkapi form berikut untuk mengajukan surat
            </p>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  {formData.noKK.length !== 16 && formData.noKK.length > 0 && (
                    <p className="text-red-500 text-xs mt-1">
                      Nomor KK harus 16 digit angka
                    </p>
                  )}
                  {kkValid === false && formData.noKK.length === 16 && (
                    <p className="text-red-500 text-xs mt-1">
                      Nomor KK tidak terdaftar di database warga
                    </p>
                  )}
                  {kkValid && wargaData && (
                ...
                <div>
                  ...
                  <input
                    ...
                    title="Harus 16 digit angka"
                  />
                  {formData.noNIK.length !== 16 &&
                    formData.noNIK.length > 0 && (
                      <p className="text-red-500 text-xs mt-1">
                        NIK harus 16 digit angka
                      </p>
                    )}
                  {nikValid === false && formData.noNIK.length === 16 && (
                    <p className="text-red-500 text-xs mt-1">
                      NIK tidak terdaftar di database warga
                    </p>
                  )}
                  {nikValid && wargaData && (
                    <div className="text-green-500 text-xs mt-1">
                      <p>NIK terdaftar atas nama: {wargaData.nama}</p>
                    </div>
                  )}
                </div>
              </div>
